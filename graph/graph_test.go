...
func TestGetNeighbors(t *testing.T) {
	...
	tests := []struct {
		...
		{[]int{2, 12, 16}, 5},
		{[]int{3, 13, 15, 17}, 6},
		{[]int{0, 4, 10, 14, 16, 18}, 7},
		{[]int{1, 11, 17, 19}, 8},
		{[]int{2, 12, 18}, 9},
		{[]int{1, 7, 17, 21}, 10},
		{[]int{0, 2, 8, 18, 20, 22}, 11},
		{[]int{1, 3, 5, 9, 15, 19, 21, 23}, 12},
		{[]int{2, 4, 6, 16, 22, 24}, 13},
		{[]int{3, 7, 17, 23}, 14},
		{[]int{6, 12, 22}, 15},
		{[]int{5, 7, 13, 23}, 16},
		{[]int{6, 8, 10, 14, 20, 24}, 17},
		{[]int{7, 9, 11, 21}, 18},
		{[]int{8, 12, 22}, 19},
		{[]int{11, 17}, 20},
		{[]int{10, 12, 18}, 21},
	...
	for _, tt := range tests {
		t.Run("pos_"+string(rune('0'+tt.pos)), func(t *testing.T) {
			...
			assert.Len(t, neighbors, len(tt.expected), "GetNeighbors(%d) length", tt.pos)
