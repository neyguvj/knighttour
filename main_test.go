...
func TestParseArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "defaults",
			args:     nil,
			expected: &appArgs{size: 5, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(5), mode: modeClass, gcPercent: counter.DefaultGCPercentReversal},
		},
		{
			name:     "explicit flags",
			args:     []string{"-size", "6", "-workers", "4", "-precompute-depth", "3"},
			expected: &appArgs{size: 6, workers: 4, precomputeDepth: 3, mode: modeClass, gcPercent: counter.DefaultGCPercentReversal},
		},
		{name: "board 5 default depth", args: []string{"-size", "5"}, expected: &appArgs{size: 5, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(5), mode: modeClass, gcPercent: counter.DefaultGCPercentReversal}},
		{name: "board 6 default depth", args: []string{"-size", "6"}, expected: &appArgs{size: 6, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(6), mode: modeClass, gcPercent: counter.DefaultGCPercentReversal}},
		{name: "board 7 default depth", args: []string{"-size", "7"}, expected: &appArgs{size: 7, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(7), mode: modeClass, gcPercent: counter.DefaultGCPercentReversal}},
		{name: "board 8 default depth", args: []string{"-size", "8"}, expected: &appArgs{size: 8, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(8), mode: modeClass, gcPercent: counter.DefaultGCPercentReversal}},
		{name: "board 8 max depth", args: []string{"-size", "8", "-precompute-depth", "32"}, expected: &appArgs{size: 8, workers: runtime.NumCPU(), precomputeDepth: 32, mode: modeClass, gcPercent: counter.DefaultGCPercentReversal}},
		...
		{name: "gc percent off", args: []string{"-gc-percent", "0"}, expected: &appArgs{size: 5, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(5), mode: modeClass, gcPercent: 0}},
		...
		{name: "depth above half board", args: []string{"-size", "5", "-precompute-depth", "13"}, wantErr: true},
		{
			...
			expected: &appArgs{size: 6, workers: runtime.NumCPU(), precomputeDepth: counter.DefaultPrecomputeDepth(6), tailMemo: 12, mode: modeClass, gcPercent: counter.DefaultGCPercentReversal},
		},
