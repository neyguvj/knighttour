...
коммит в ушедший вперёд `main`. Сначала зафиксировать фичу в feature-ветке. Канон любой коммит-инвокации —
одна цепочка `make check && git add <файлы> && git commit -m …`: аттестующий чек не пайпить (`| tee`,
`| tail` крадут exit code — штамп не ставится; редиректы `> log 2>&1` легальны); заблокированная
инвокация не исполняет ничего, включая пристыкованный `git add`. `main` занят основным деревом, потому
слияние идёт через него:
