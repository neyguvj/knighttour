...
- Завершившийся контекст — внешняя отмена **или** истечение deadline (`ctx.Err() != nil`) — на
  ...
  без паники. Прочая ошибка `Each` недостижима по контракту колбэка (`specs/cache.md`) и
  означает баг обхода — panic.
...
публикует hits/misses в счётчики фазы `counting`. Завершившийся контекст (отмена и дедлайн —
таблица) даёт частичный итог без паники. GC-ручка (ADR-014): итог не зависит от
