...
3. **counting** — обход-продюсер + общий bounded LIFO-стек копий (план 13):
   ...
   не растёт; `K` — ручка глубины на воркера, по умолчанию 10000 — измеренная лучшая точка
   свипа на 7×7 d22 (план 13).
   Эффективный батч `Beff = min(B, max(1, записей/(консьюмеры·C)))` считается один раз на старте
   фазы из `ItemsCount()` task-cache, до создания стека; `B` — потолок батча (по умолчанию 16),
